...
ILOLAZYCONSTRAINTCALLBACK1(LazyMixedBfsCallback, IloIntVarArray, x)
{
	lazyCall++;
	IloEnv env = getEnv();
	...
	for(int i=0; i<nComp; i++){
		...
		if((dim==cap+2 && connectivityValues[i]==1) || (dim>cap+2)){
			compConstr=x[0] - x[0];
			mixedBFS(compVertices[i][0], i);

			for(int j=0; j<dim; j++){
				int vIndexComp=compVertices[i][j];
				IloInt maxDim = findCompCoeff(dim, vIndexComp, i);
...
ILOUSERCUTCALLBACK1(UserMixedBfsCallback, IloIntVarArray, x){
...
void MixedModelBFS()
{
	...
	try{
		...
		sec = cplex.use(LazyMixedBfsCallback(env, x));

		IloCplex::Callback cut;
		if(userFreq>0)
			cut = cplex.use(UserMixedBfsCallback(env, x));
		...
		if(cplex.getStatus() == IloAlgorithm::Unknown || cplex.getStatus() == IloAlgorithm::Infeasible){
			...
			if(output.is_open())
			{
				...
				output << "null" << "\t" << bound << "\t" << time << "\t" << cplex.getStatus() << "\t" << nVar << "\t"  << nConst << "\t" << nNode
					   << "\t" << userCuts << "\t" << compCuts << "\t" << kConnCuts << "\t" <<  bppCuts << "\t" << lazyCall;
			}
		}
		else{
